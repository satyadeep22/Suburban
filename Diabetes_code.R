...
suburbandata <- sqlQuery(myconn, "select RecordNo, REG_CENTRE_NAME,
                                  PROCESS_CENTRE_NAME, REG_DT, CID_NO, BAR_CODE,
                                  SID, AGE, GENDER, TEST_NAME, COMPONENT_NAME, 
                                  RESULT_VALUE, [NORMAL_VALUE(B/W-RANGE)] as 
                                  NORMAL_VALUE 
                                  from dbo.Services_stats 
                                  where TEST_NAME IN ('LIPID-P', 
                                  'GLYCO Hb (HbA1C)', 'FBS (PLASMA)', 'PPBS (PLASMA)') 
                                  ")


All_Test <- read.csv("All_Test_Normal_Values.csv")
All_Test <- All_Test[(All_Test$TEST_NAME %in% c('LIPID-P', 'GLYCO Hb (HbA1c)',
                                                'FBS (PLASMA)' , 
                                                'PPBS (PLASMA)')),]
All_Test <- na.omit(All_Test)

All_Test <- All_Test[which(All_Test$no_rows >100),]
All_Test <- All_Test[!All_Test$COMPONENT_NAME %in% c('CHOL / HDL CHOL RATIO',
...
names(suburbandata)[names(suburbandata)=="NORMAL_VALUE"] <- 
                                                      "NORMAL_VALUE.B.W.RANGE."

merged_suburbandata <- merge(All_Test,suburbandata, by = c("TEST_NAME", 
                                                           "COMPONENT_NAME", 
                                                           "NORMAL_VALUE.B.W.RANGE." 
                                                           )
                             )
...
merged_suburbandata <- merged_suburbandata[!merged_suburbandata$REG_CENTRE_NAME
                                           %in% c( 'Wani Diagnsotics', 
                                                'Bhakti Diagnostics - Latur', 
                                                'Goa Lab', 'Goa Navelim', 
                                                'Goa Panjim', 'Goa Parvorim', 
                                                'Goa ponda', 'Goa-Margao', 
...
merged_suburbandata$RESULT_VALUE <- 
                    as.numeric(as.character(merged_suburbandata$RESULT_VALUE))

merged_suburbandata$NORMAL_LOWER_VALUE <- 
                    as.numeric(as.character(merged_suburbandata$NORMAL_LOWER_VALUE))

merged_suburbandata$NORMAL_UPPER_VALUE <- 
                    as.numeric(as.character(merged_suburbandata$NORMAL_UPPER_VALUE))

merged_suburbandata$POSITIVITY <- 
                    ifelse(merged_suburbandata$RESULT_VALUE < merged_suburbandata$NORMAL_LOWER_VALUE, 1,
...
library(sqldf)
diabetes_CID <- sqldf("select MONTH, REG_CENTRE_NAME, AGE_GROUP, GENDER, 
                       COUNT(DISTINCT(CID_NO)) as TEST_CID from diabetes_data
...
diabetes_reg_pivot <- sqldf('select MONTH, REG_CENTRE_NAME, AGE_GROUP, GENDER, 
                             SID, SUM(POSITIVITY) as SUM_POSITIVITY 
                             from diabetes_reg
                             group by MONTH, REG_CENTRE_NAME, AGE_GROUP, GENDER, SID 
                             order by MONTH, REG_CENTRE_NAME, AGE_GROUP, GENDER, SID')

diabetes_reg_pivot$diabetes_patient <- 
                      ifelse(diabetes_reg_pivot$SUM_POSITIVITY >0 ,1,0)

diabetes_sample <- sqldf("select * from diabetes_data where SID NOT like '1%' ")

diabetes_sample_pivot <- sqldf('select MONTH, REG_CENTRE_NAME, AGE_GROUP, GENDER, 
                                SID, SUM(POSITIVITY) as SUM_POSITIVITY 
                                from diabetes_sample
                                group by MONTH, REG_CENTRE_NAME, AGE_GROUP, GENDER, SID 
                                order by MONTH, REG_CENTRE_NAME, AGE_GROUP, GENDER, SID')

diabetes_sample_pivot$diabetes_patient <- 
                            ifelse(diabetes_sample_pivot$SUM_POSITIVITY >0 ,1,0)
...
final_diabetes <- merge(final_diabetes_reg,final_diabetes_sample, 
                        by = c("MONTH","REG_CENTRE_NAME", "AGE_GROUP", "GENDER" ), 
                        all = TRUE)
...
Overall_data <- merge(final_diabetes, AllData, 
                      by = c("MONTH","REG_CENTRE_NAME", "AGE_GROUP", "GENDER" ), 
                      all = TRUE)
...
Overall_data$DIABETES_TEST_TAKEN <-  
              Overall_data$REGISTERED_DIABETES+Overall_data$SAMPLE_TAKEN_DIABETES
Overall_data$DIABETES_POSITIVE <- 
              Overall_data$REGISTERED_POSITIVE_DIABETES+Overall_data$SAMPLE_POSITIVE_DIABETES
...
Final_Diabetes_Report <- sqldf('select MONTH, REG_CENTRE_NAME, AGE_GROUP, GENDER,
                                TOTAL_CID, TOTAL_SID, REGISTERED_DIABETES, 
                                REGISTERED_POSITIVE_DIABETES, SAMPLE_TAKEN_DIABETES, 
                                SAMPLE_POSITIVE_DIABETES, DIABETES_TEST_TAKEN, 
                                DIABETES_POSITIVE
                                from Overall_data')
...
ward_demographics <- sqldf("select WARD, SUM(MALE) as TOTAL_MALE, SUM(FEMALE) 
                            as TOTAL_FEMALE, 
                            SUM(TOTAL_POPULATION) as TOTAL_POPULATION, 
                            SUM(AREA_SKM) as AREA_SKM 
                            from Mumbai_Overall 
